...
- 从本工具点击“启动程序”时，会在目标进程运行前完成亲和度绑定。
- 管理员模式下使用 WMI 进程启动事件。
...
对于带内核保护的进程，如果运行后再设置亲和度会返回“拒绝访问”，可从本工具
点击“启动程序”。工具会先以挂起状态创建目标进程，在它开始执行前设置亲和度，
然后立即恢复运行。该流程只使用 Windows 标准进程创建 API，不注入、不修改内存、
不安装 Hook。反作弊策略由游戏厂商决定，受保护进程是否允许启动时绑定最终仍以
其反作弊规则为准。
...
```text
dist\CcdAffinityManager-1.0.2-portable.zip
dist\CcdAffinityManager-Setup-1.0.2.exe
```
